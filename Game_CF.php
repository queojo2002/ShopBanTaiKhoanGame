	...
	<div class="m-l-10 m-r-10">
		<form  class="form-inline m-b-10" role="form" method="get">
			...
			<div class="col-md-3 col-sm-4 p-5 no-radius">
				...
				<a style = "text-align: center; padding: 12px 20px 4px 20px;" class="btn c-square m-l-0 btn-danger" href="Game_CF.php">Xóa Tìm Kiếm</a>
			</div>
		</form>
	</div>
</div>
...
if (isset($_GET['price']) And $_GET['price'] <> "") {
	$dataacc = mysqli_query($conn, "SELECT * FROM posts WHERE $sql_gia And account_type = 'CF' AND status_posts = 'Chưa Bán' ORDER by id DESC LIMIT $start_from, $num_rec_per_page");
	$tong = mysqli_result(mysqli_query($conn,"SELECT COUNT(*) FROM `posts` WHERE $sql_gia And `account_type` = 'CF' AND status_posts = 'Chưa Bán'"), 0);
}else {
	$dataacc = mysqli_query($conn, "SELECT * FROM posts WHERE account_type = 'CF' AND status_posts = 'Chưa Bán' ORDER by id DESC LIMIT $start_from, $num_rec_per_page");
	$tong = mysqli_result(mysqli_query($conn,"SELECT COUNT(*) FROM `posts` WHERE  `account_type` = 'CF' AND status_posts = 'Chưa Bán'"), 0);
}
	...
	echo '
			...
			<div class="attribute_info">
				<div class="row">
					...
					<div class="col-xs-12 a_att">Số Vip Sỡ Hữu: <b>'.$row["info_sovip_cf"].'</b> </div>
				</div>
			</div>
			<div class="a-more">
				<div class="row">
