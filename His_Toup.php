...
$_SESSION['title'] = "Lịch Sử Nạp Thẻ";
...
<?php 
	$data_his_toup = mysqli_query($conn,"SELECT * FROM `his_toup` WHERE `username_login_toup` = '".$username_login."' ORDER BY id DESC");
	while($row = mysqli_fetch_array($data_his_toup))
	{
		echo '
				<thead>
					<tr>
						<td>'.$row["id"].'</td>
						<td>'.$row["time_toup"].'</td>
						<td>'.$row["info_toup_type"].'</td>
						<td>'.$row["info_loaithe"].'</td>
						<td>'.$row["info_mathe"].'/'.$row["info_seri"].'</td>
						<td>'._tien($row["info_menhgia"]).'</td>
						<td>'.$row["result_toup"].'</td>
						<td>'._tien($row["info_thucnhan"]).'</td>
					</tr>
				</thead>
		';
	}
